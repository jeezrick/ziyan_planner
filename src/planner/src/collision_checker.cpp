...
GridCollisionChecker::GridCollisionChecker(
  std::shared_ptr<ZiYan_IO::Costmap2DZiYan> costmap_zy,
  unsigned int num_quantizations,
  ZiYan_IO::Info::SharedPtr node)
: FootprintCollisionChecker(costmap_zy ? costmap_zy->getCostmap() : nullptr)
{
  if (costmap_zy) {
    costmap_zy_ = costmap_zy;
  }
  ...
  const double & possible_collision_cost)
{
  ...
  if (footprint == unoriented_footprint_) {
    #ifdef COLLISION_CHECKER_DEBUG_
    ZIYAN_INFO("Footprint unchanged, no updates required");
    #endif
