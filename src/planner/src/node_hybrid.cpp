...
#include "planner/node_hybrid.hpp"
...
std::shared_ptr<ZiYan_IO::Costmap2DZiYan> NodeHybrid::costmap_ziyan = nullptr;
  ...
  SearchInfo & search_info)
{
    ...
    motion_model == MotionModel::DUBIN)
  {
    ...
    ZIYAN_INFO("Motion model is the same, no need to recompute primitives");
    #endif
  ...
  /* wonderful's note */
  /* sqrt(2) <= 2 * R * sin (angle / 2) -> angle >= 2.0 * asin(sqrt(2) / (2 * R)) */
  ...
  /* wonderful's note */
  /* it maybe bigger than sqrt(2), since angle is the least increment of bin_size that is bigger than original angle */
  ...
  #ifdef NODE_HYBRID_DEBUG_
  // vis delta_xs delata_ys
  for (unsigned int i = 0; i != projections.size(); i++) {
    for (unsigned int j = 0; j != num_angle_quantization; j++) {
      ZIYAN_INFO(
        "Projection %d, Angle %d: x: %.2f, y: %.2f", i, j, delta_xs[i][j], delta_ys[i][j]);
    }
  }
  #endif
  ...
  for (unsigned int i = 0; i != projections.size(); i++) {
    ...
    if (turn_dir != TurnDirection::FORWARD && turn_dir != TurnDirection::REVERSE) {
      ...
      const float turning_rad = delta_dist / (2.0f * sin(angle / 2.0f)); // radius
      travel_costs[i] = turning_rad * angle; // curve length
    } else {
...
{
  const float normalized_cost = child->getCost() / 252.0f; // why
...
{
  // return std::sqrt(
  //   (node_coords.x - goal_coords.x) * (node_coords.x - goal_coords.x) +
  //   (node_coords.y - goal_coords.y) * (node_coords.y - goal_coords.y));
...
void NodeHybrid::resetObstacleHeuristic(
  std::shared_ptr<ZiYan_IO::Costmap2DZiYan> costmap_ziyan_i,
  ...
  const unsigned int & goal_x, const unsigned int & goal_y)
{
  ...
  costmap_ziyan = costmap_ziyan_i;
  inflation_layer = costmap_ziyan->getInflationLayer();
  sampled_costmap = costmap_ziyan->getCostmap();
  if (motion_table.downsample_obstacle_heuristic) {
    ZiYan_IO::Info::WeakPtr ptr;
  ...
  #ifdef NODE_HYBRID_DEBUG_
  std::cout << "Resetting obstacle heuristic -> downsample_obstacle_heuristic: " <<  (motion_table.downsample_obstacle_heuristic ? "true" : "false") << std::endl;
  std::cout << "reset size: " << size << std::endl;
  #endif
}
...
float NodeHybrid::adjustedFootprintCost(const float & cost)
{
  ...
  const auto inflation_layer = costmap_ziyan->getInflationLayer();
  const float scale_factor = inflation_layer->getCostScalingFactor();
  const float min_radius = inflation_layer->getInscribedRadius();
  ...
  return static_cast<float>(
    inflation_layer->computeCost(dist_to_obj / costmap_ziyan->getCostmap()->getResolution()));
}
  ...
  const float & cost_penalty)
{
  ...
  const unsigned int size_x = sampled_costmap->getSizeInCellsX();
  const bool is_circular = costmap_ziyan->getUseRadius();
...
{
  #ifdef NODE_HYBRID_DEBUG_
  ZIYAN_INFO("cur node -> x: %.2f, y: %.2f theta: %.2f", this->pose.x, this->pose.y, this->pose.theta);
  #endif
