  ...
  const unsigned int & dim_3_size)
{
  ...
  #ifdef ASTAR_DEBUG_
  ZIYAN_INFO(
    "Initializing A* with traverse unknown: %s, motion_model: %s, "
    "max_iterations: %d, on approach iterations: %d, "
    "terminal checking interval: %d, max planning time: %f, "
    "lookup table size: %f, dim 3 size: %d",
    allow_unknown ? "true" : "false",
    toString(_motion_model).c_str(),
    max_iterations, max_on_approach_iterations, terminal_checking_interval, 
    max_planning_time, lookup_table_size, dim_3_size
  );
  #endif
...
{
  #ifdef ASTAR_DEBUG_
  ZIYAN_INFO("Creating path with tolerance: %f", tolerance);
  #endif
  ...
  while (iterations < getMaxIterations() && !_queue.empty()) {
    ...
    // // Save current node coordinates for debug TODO: align with python log
    // if (expansions_log) {
    //   populateExpansionsLog(current_node, expansions_log);
    // }

    // We allow for nodes to be queued multiple times in case
    // shorter paths result in it, but we can visit only once /* Meaning h(n) is consistent? */
