...
typedef std::vector<ZiYan_IO::PoseStamped>::iterator PathIterator;
typedef std::vector<ZiYan_IO::PoseStamped>::reverse_iterator ReversePathIterator;
...
protected:
  ...
  inline double getFieldByDim(
    const ZiYan_IO::PoseStamped & msg,
   ...
   */
  std::vector<PathSegment> findDirectionalPathSegments(const ZiYan_IO::Path & path);
