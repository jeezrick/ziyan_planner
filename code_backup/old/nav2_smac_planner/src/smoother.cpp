  ...
  const double & max_time)
{
  ...
  ZiYan_IO::Path curr_path_segment;
  ...
  for (unsigned int i = 0; i != path_segments.size(); i++) {
    if (path_segments[i].end - path_segments[i].start > 10) {
      ...
      const ZiYan_IO::Pose start_pose = curr_path_segment.poses.front().pose;
      const ZiYan_IO::Pose goal_pose = curr_path_segment.poses.back().pose;
      bool local_success =
  ...
  const double & max_time)
{
  ...
  ZiYan_IO::Path new_path = path;
  ZiYan_IO::Path last_path = path;
  ...
  while (change >= tolerance_) {
    ...
    duration<double> time_span_dur = duration_cast<duration<double>>(b - a);
    double timespan = static_cast<double>(time_span_dur.count());
...
double Smoother::getFieldByDim(
  const ZiYan_IO::PoseStamped & msg, const unsigned int & dim)
{
  if (dim == 0) {
...
void Smoother::setFieldByDim(
  ZiYan_IO::PoseStamped & msg, const unsigned int dim,
  const double & value)
{
  if (dim == 0) {
...
std::vector<PathSegment> Smoother::findDirectionalPathSegments(const ZiYan_IO::Path & path)
{
  ...
  for (unsigned int idx = 1; idx < path.poses.size() - 1; ++idx) {
    ...
    double cur_theta = ZiYan_IO::getYaw(path.poses[idx].pose.orientation);
    double next_theta = ZiYan_IO::getYaw(path.poses[idx + 1].pose.orientation);
    double dtheta = shortest_angular_distance(cur_theta, next_theta);
  ...
  bool & reversing_segment)
{
  ...
  theta = atan2(dy, dx);
  pt_yaw = ZiYan_IO::getYaw(path.poses[1].pose.orientation);
  if (!is_holonomic_ && fabs(shortest_angular_distance(pt_yaw, theta)) > M_PI_2) {
...
void Smoother::findBoundaryExpansion(
  const ZiYan_IO::Pose & start,
  const ZiYan_IO::Pose & end,
  ...
  const nav2_costmap_2d::Costmap2D * costmap)
{
  ...
  from[2] = ZiYan_IO::getYaw(start.orientation);
  to[0] = end.position.x;
  to[1] = end.position.y;
  to[2] = ZiYan_IO::getYaw(end.orientation);
...
void Smoother::enforceStartBoundaryConditions(
  const ZiYan_IO::Pose & start_pose,
  ZiYan_IO::Path & path,
  const nav2_costmap_2d::Costmap2D * costmap,
