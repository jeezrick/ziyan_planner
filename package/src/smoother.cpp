  ...
  const double & max_time)
{
  ...
  for (unsigned int i = 0; i != path_segments.size(); i++) {
    if (path_segments[i].end - path_segments[i].start > 10) {
      ...
      const Pose start_pose = curr_path_segment.poses.front().pose;
      const Pose goal_pose = curr_path_segment.poses.back().pose;
      bool local_success =
...
double Smoother::getFieldByDim(
  const PoseStamped & msg, const unsigned int & dim)
{
  if (dim == 0) {
...
std::vector<PathSegment> Smoother::findDirectionalPathSegments(const Path & path)
{
  ...
  for (unsigned int idx = 1; idx < path.poses.size() - 1; ++idx) {
    ...
    double cur_theta = getYaw(path.poses[idx].pose.orientation);
    double next_theta = getYaw(path.poses[idx + 1].pose.orientation);
  ...
  bool & reversing_segment)
{
  ...
  theta = atan2(dy, dx);
  pt_yaw = getYaw(path.poses[1].pose.orientation);
  ...
  const ziyan_costmap::Costmap2D * costmap)
{
  ...
  from[2] = getYaw(start.orientation);
  to[0] = end.position.x;
  to[1] = end.position.y;
  to[2] = getYaw(end.orientation);
